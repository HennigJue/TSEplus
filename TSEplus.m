...
for kseq=1
    ...
    acqP.fov=220e-3;
    acqP.Nx=300;        acqP.Ny=126;        acqP.PEfac=acqP.Nx/acqP.Ny; %=1:same fov; =acqP.Nx/acqP.Ny:same res
    acqP.necho=128;
    acqP.samplingTime= 5.4e-3;                                           %should be a multiple of Nx in microseconds
    acqP.NSlices=1;     acqP.sliceGAP=1.2;  acqP.sliceThickness=2e-3;
    acqP.flipref=120;   acqP.flipflag=3;
    acqP.TE=10e-3;      acqP.TEeff=10e-3;   acqP.TEprep=30e-3; acqP.dTE=0e-3;
    acqP.TR=4.66;       acqP.TI=0e-3;      acqP.TImod=1;
    acqP.HF_fac=0.9;      acqP.PEref=11;      acqP.GR_fac=1;
    acqP.PEtype='centric';
    ...
    if(acqP.navmode(2)==1)
        ...
        temp(1:k0-1,:)=acqP.PEorder(1:k0-1,:);
        temp(k0+1:end,:)=acqP.PEorder(k0:end,:);
        acqP.PEorder=temp;
    ...
    if(acqP.navmode(3)>0), acqP.necho=acqP.necho+1; end;
